 ...
 *
 * This file is mostly a direct translation of the [Docco source](https://github.com/jashkenas/docco/blob/c95b417d/docco.litcoffee), which is licensed under MIT and Copyright (c) 2009-2013 Jeremy Ashkenas.
 */
...
var Doxxo = module.exports = {
	 ...
	 * ### run()
	 *
	 * This defines the interface that Doxxo uses to run from the command line. Options are parsed using [Commander](https://github.com/tj/commander.js) and passed to `Doxxo.document()`.
	 *
	 * #### Arguments
	 *
	...
	run: function run(args) {
		...
		commander.version(Doxxo.version)
			...
			.option('-i, --index [file]',     'mark a file as the index file', defaults.index)
			.parse(args)
			.name = "doxxo"
	 ...
	 *   - **options.sources** _array_ - An array of paths to JavaScript files, relative to the current working directory.
	 *   - **options.output** _string_ - The path to a folder that generated files will be saved in.
	 *   - **options.layout** _string_ - The name of the built-in layout to use. Valid values are `parallel`, `linear`, `classic`, `plain-markdown` or `bti`.
	 *   - **options.marked** _object_ - Options to configure the markdown parser and renderer.
	 *   - **options.template** _string_ - A path to an Underscore style template file.
	 *   - **options.css** _string_ - A path to a CSS file to use for styling.
	 *   - **options.index** _string_ - A path to the source file that should be considered the index file. This path should be the same path passed to sources.
	 * - **callback** _function; optional_ - A function that is called when the process is complete. If an error occurs, callback will be given the Error object as an argument.
	 */
	 ...
	 *   - **config.marked** _object_ - Options to configure the markdown output. This value is passed directly `marked()`. The default value is `{ smartypants: true, breaks: false }`.
	 */
	format: function format(source, sections, config) {
		var markedOptions = _.clone(config.marked || {
			smartypants: true,
			breaks: false
		});
	 ...
	 * #### Arguments
	 *
	 * - **options** _object_ - An object of options to configure this run of Doxxo.
	 */
	configure: function configure(options) {
		...
		config.sources = options.args.sort().map(function(src) {
			return path.relative(".", path.normalize(src));
		});

		if (config.index) config.index = path.relative(".", path.normalize(config.index));
