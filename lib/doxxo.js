...
module.exports = function(paths, opts, callback) {
	...
	.then(function() {
		...
		return doxxo.copyLayoutAssets(opts.layout, opts.output).then(function(to) {
			opts.log("doxxo: Copied layout assets to '%s'", path.relative(cwd, to));
		});
	})
	...
	.then(function() {
		return Promise.each(sources, function(src) {
			return doxxo.document(src, opts).then(function() {
				opts.log("doxxo: %s -> %s", path.relative(cwd, src.full), path.relative(cwd, src.out));
			});
		}, { concurrency: 1 });
	})
...
doxxo.configure = function(opts) {
	...
	opts.log = function() {
		if (!opts.silent) console.log.apply(console, arguments);
	}
	return opts;
}
